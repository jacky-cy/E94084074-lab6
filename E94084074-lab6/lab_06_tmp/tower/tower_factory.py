...
class Tower:
    ...
    def PCR(cls, x: int, y: int):
        ...
        :return: object
        """
                                                          #use inheritance to creat the new tower and its properties
        pcr = cls(x, y, Snipe(), PCR_IMAGE)
        pcr._range = [100, 105, 110, 115, 120, 125]       #set the range
        pcr._damage =  [3.0, 3.2, 3.4, 3.6, 3.8, 4.0]     #set the damage
        pcr.cd_max_count=120                              #pcr.cd_max_count is 120 fps
        return pcr
