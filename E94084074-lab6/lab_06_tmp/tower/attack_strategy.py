...
class Snipe(AttackStrategy):
    ...
    def attack(self, enemies, tower, cd_count):     #use inheritance to make attack  
        for en in enemies.get():                    #check every enermy in_range
            if in_range(en, tower):                 #eliminate an enemy in once
                en.health = 0
                cd_count = 0                        #reset the cd_count
                return cd_count
        return cd_count
